...
interface queries_count_type {
  kafka_cluster_partition_insyncreplicascount: object[],
  kafka_cluster_partition_underreplicated: object[],
  kafka_controller_kafkacontroller_offlinepartitionscount: object[],
  kafka_network_requestmetrics_requestbytes_count: object[],
  kafka_controller_kafkacontroller_activebrokercount: object[],
  kafka_controller_kafkacontroller_activecontrollercount: object[]
}

const queries_count: queries_count_type = {
...
const queries_chart: queries_chart_type = {
...
function query(socket: object, ip: string){
...
function query_count(socket: object, ip: string){
  console.log(ip)
  for (const key of Object.keys(queries_count)) {
    axios.get(`http://${ip}/api/v1/query`,{ 
      ...
      .then(res => {
        ...
        const retrieved_data: object[] = [];
        res.data.result.forEach((x: { metric: { instance: string; job: string; partition: string; topic: string; request: string; }; value: any[]; }) => {
          const obj = {};
...
function query_chart(socket, ip = ipInCache, range: number = 15){
